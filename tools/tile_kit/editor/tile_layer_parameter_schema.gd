...
const KIND_ORDER := [
	...
	"sprite_tufts", "clay_tufts", "fringe", "fence",
]
...
const DEFINITIONS := {
	...
	"clay_tufts": {
		"label": "Clay Sprout Carpet",
		"description": "Solid egg-lobe sprout rosettes on an offset grid — chunky clay volume.",
		"defaults": {"placement": "accents", "primary_key": "grass_gg_tuft",
			"root_key": "grass_gg_tuft",
			"accent_count": [4, 6], "accent_min_spacing": 0.45,
			"grid_spacing": 0.195, "row_offset_fraction": 0.5,
			"tuft_footprint": [0.17, 0.24], "height_ratio": 0.9,
			"position_jitter": 0.045, "lobe_count": [3, 4],
			"splay_degrees": [30.0, 40.0], "edge_margin": 0.18,
			"root_sink": 0.03, "skip_fraction": 0.0},
		"parameters": [
			{"section": "PLACEMENT", "key": "placement", "label": "Composition",
				"type": "enum", "values": ["accents", "grid"],
				"value_labels": ["Sparse accents", "Periodic pile"],
				"randomize": false},
			{"key": "accent_count", "label": "Sprout count", "type": "range_int",
				"min": 1, "max": 12, "step": 1, "fallback": [4, 6],
				"when": {"placement": ["accents"]}},
			{"key": "accent_min_spacing", "label": "Sprout spacing", "type": "float",
				"min": 0.20, "max": 0.90, "step": 0.01,
				"when": {"placement": ["accents"]}},
			{"section": "CARPET GRID", "key": "grid_spacing", "label": "Tuft spacing",
				"type": "float", "min": 0.12, "max": 0.70, "step": 0.005,
				"when": {"placement": ["grid"]}},
			{"key": "row_offset_fraction", "label": "Row offset", "type": "float",
				"min": 0.0, "max": 0.5, "step": 0.05},
			{"key": "position_jitter", "label": "Placement jitter", "type": "float",
			...
			{"key": "edge_margin", "label": "Rim margin", "type": "float",
				"min": 0.0, "max": 0.30, "step": 0.005},
			{"section": "SPROUT SHAPE", "key": "tuft_footprint", "label": "Sprout size",
				"type": "range_float", "min": 0.10, "max": 0.60, "step": 0.005,
				"fallback": [0.16, 0.21]},
			{"key": "height_ratio", "label": "Height scale", "type": "float",
				"min": 0.45, "max": 1.6, "step": 0.01},
			{"key": "lobe_count", "label": "Leaves per sprout", "type": "range_int",
				"min": 3, "max": 7, "step": 1, "fallback": [3, 4]},
			{"key": "splay_degrees", "label": "Leaf splay", "type": "range_float",
				"min": 12.0, "max": 60.0, "step": 1.0, "fallback": [26.0, 36.0]},
			{"key": "root_sink", "label": "Root sink", "type": "float",
				"min": 0.0, "max": 0.08, "step": 0.005},
		],
	},
