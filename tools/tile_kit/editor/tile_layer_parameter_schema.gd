...
const KIND_ORDER := [
	...
	"sprite_tufts", "clay_sprouts", "rooted_meadow", "forest_floor",
	"fringe", "fence",
]
...
const DEFINITIONS := {
		...
		"parameters": [
			...
			{"section": "SURFACE RELIEF", "key": "relief_style", "label": "Relief style",
				"type": "enum", "values": ["none", "pillow", "dunes",
					"sculpted_dunes", "natural_dunes", "contour_moss", "heaps", "furrows"],
				"value_labels": ["None", "Soft pillow", "Dunes", "Sculpted dunes",
					"Natural dunes", "Clay moss contours", "Heaps", "Furrows"], "randomize": false},
			...
			{"section": "DUNE FORM", "key": "dune_scale", "label": "Dune scale",
				"type": "float", "min": 0.35, "max": 1.25, "step": 0.01,
				"when": {"relief_style": ["sculpted_dunes", "natural_dunes"]}},
	...
	"clutter": {
		...
		"defaults": {"count": [7, 13], "diameter": [0.05, 0.11],
			...
			"cluster_radius": 0.24, "edge_fraction": 0.0,
			"corner_fraction": 0.0},
		"parameters": [
			...
			{"key": "cluster_radius", "label": "Group radius", "type": "float",
				...
				"when": {"placement_mode": ["clusters"]}},
			{"key": "edge_fraction", "label": "Edge coverage", "type": "float",
				"min": 0.0, "max": 0.7, "step": 0.01,
				"when": {"placement_mode": ["clusters"]}},
			{"key": "corner_fraction", "label": "Corner coverage", "type": "float",
				"min": 0.0, "max": 0.35, "step": 0.01,
				"when": {"placement_mode": ["clusters"]}},
	...
	"rooted_meadow": {
		"label": "Rooted Meadow",
		"description": "Clay grass fans, slim ferns, round clovers, and optional flower varieties on one shared wind surface.",
		"defaults": {
			"turf_count": [9, 9], "fern_count": [1, 1],
			"clover_count": [3, 3], "flower_count": [0, 0],
			"blades_per_turf": [3, 5], "grass_height": [0.1257, 0.2214],
			"blade_width": [0.082411676, 0.106563553],
			"clover_size": [0.05724, 0.063],
			"four_leaf_chance": 0.28, "accent_flower_chance": 0.0,
			"edge_margin": 0.025,
			"min_spacing": 0.15, "root_sink": 0.010,
		},
		"parameters": [
			{"section": "COMPOSITION", "key": "turf_count",
				"label": "Grass turfs", "type": "range_int", "min": 0,
				"max": 18, "step": 1, "fallback": [9, 9]},
			{"key": "fern_count", "label": "Small ferns",
				"type": "range_int", "min": 0, "max": 4, "step": 1,
				"fallback": [1, 1]},
			{"key": "clover_count", "label": "Clover clusters",
				"type": "range_int", "min": 0, "max": 8, "step": 1,
				"fallback": [3, 3]},
			{"key": "flower_count", "label": "Tiny flowers",
				"type": "range_int", "min": 0, "max": 8, "step": 1,
				"fallback": [0, 0]},
			{"key": "accent_flower_chance", "label": "Pink cup chance",
				"type": "float", "min": 0.0, "max": 1.0, "step": 0.01},
			{"key": "min_spacing", "label": "Organic spacing", "type": "float",
				"min": 0.08, "max": 0.34, "step": 0.005},
			{"key": "edge_margin", "label": "Extra edge padding", "type": "float",
				"min": 0.0, "max": 0.18, "step": 0.005},
			{"section": "GRASS SHAPE", "key": "blades_per_turf",
				"label": "Blades per turf", "type": "range_int", "min": 1,
				"max": 6, "step": 1, "fallback": [3, 5]},
			{"key": "grass_height", "label": "Grass height",
				"type": "range_float", "min": 0.05, "max": 0.32,
				"step": 0.001, "fallback": [0.1257, 0.2214]},
			{"key": "blade_width", "label": "Blade width",
				"type": "range_float", "min": 0.025, "max": 0.12,
				"step": 0.001, "fallback": [0.082411676, 0.106563553]},
			{"key": "root_sink", "label": "Root sink", "type": "float",
				"min": 0.0, "max": 0.05, "step": 0.002},
			{"section": "CLOVER SHAPE", "key": "clover_size",
				"label": "Round leaf size", "type": "range_float",
				"min": 0.035, "max": 0.11, "step": 0.001,
				"fallback": [0.05724, 0.063]},
			{"key": "four_leaf_chance", "label": "Four-leaf chance",
				"type": "float", "min": 0.0, "max": 1.0, "step": 0.01},
		],
	},
	"forest_floor": {
		"label": "Clover & Fern Forest Floor",
		"description": "Dense low clay clover beneath a few large radial fern crowns on one shared wind surface.",
		"defaults": {
			"clover_count": [20, 28], "fern_count": [2, 4],
			...
			"fronds_per_fern": [5, 7], "leaflet_pairs": [4, 5],
			"edge_margin": 0.06, "clover_spacing": 0.095,
			"fern_spacing": 0.30, "root_sink": 0.010,
			"leaf_litter_count": [5, 8], "twig_count": [2, 3],
			"leaf_litter_size": [0.080, 0.140],
			"twig_length": [0.17, 0.28], "branch_forks": [1, 2],
			"raised_offshoot_chance": 0.48, "litter_spacing": 0.075,
		},
		"parameters": [
			{"section": "COMPOSITION", "key": "clover_count",
				"label": "Clover clusters", "type": "range_int",
				"min": 0, "max": 42, "step": 1, "fallback": [20, 28]},
			{"key": "fern_count", "label": "Large fern crowns",
				"type": "range_int", "min": 0, "max": 7, "step": 1,
				"fallback": [2, 4]},
			{"key": "edge_margin", "label": "Contained rim margin",
				"type": "float", "min": 0.02, "max": 0.22, "step": 0.005},
			{"key": "clover_spacing", "label": "Clover spacing",
				"type": "float", "min": 0.045, "max": 0.22, "step": 0.005},
			{"key": "fern_spacing", "label": "Fern spacing",
				"type": "float", "min": 0.22, "max": 0.70, "step": 0.01},
			{"section": "CLOVER CARPET", "key": "clover_size",
				"label": "Leaf size", "type": "range_float",
				"min": 0.035, "max": 0.11, "step": 0.001,
				"fallback": [0.040, 0.085]},
			{"key": "clover_height", "label": "Clover height",
				"type": "range_float", "min": 0.008, "max": 0.075,
				"step": 0.001, "fallback": [0.018, 0.032]},
			{"key": "four_leaf_chance", "label": "Four-leaf chance",
				"type": "float", "min": 0.0, "max": 1.0, "step": 0.01},
			{"section": "FERN CROWNS", "key": "fern_scale",
				"label": "Overall fern size", "type": "range_float",
				"min": 0.55, "max": 1.50, "step": 0.01,
				"fallback": [0.72, 1.30]},
			{"key": "fern_height", "label": "Fern height",
				"type": "range_float", "min": 0.09, "max": 0.36,
				"step": 0.005, "fallback": [0.17, 0.25]},
			{"key": "fern_reach", "label": "Frond reach",
				"type": "range_float", "min": 0.14, "max": 0.44,
				"step": 0.005, "fallback": [0.25, 0.34]},
			{"key": "fronds_per_fern", "label": "Fronds per crown",
				"type": "range_int", "min": 3, "max": 9, "step": 1,
				"fallback": [5, 7]},
			{"key": "leaflet_pairs", "label": "Leaflet pairs",
				"type": "range_int", "min": 3, "max": 7, "step": 1,
				"fallback": [4, 5]},
			{"key": "root_sink", "label": "Root sink", "type": "float",
				"min": 0.0, "max": 0.05, "step": 0.002},
			{"section": "FOREST LITTER", "key": "leaf_litter_count",
				"label": "Fallen leaves", "type": "range_int",
				"min": 0, "max": 20, "step": 1, "fallback": [5, 8]},
			{"key": "twig_count", "label": "Small branches",
				"type": "range_int", "min": 0, "max": 8, "step": 1,
				"fallback": [2, 3]},
			{"key": "leaf_litter_size", "label": "Leaf size",
				"type": "range_float", "min": 0.04, "max": 0.18,
				"step": 0.005, "fallback": [0.080, 0.140]},
			{"key": "twig_length", "label": "Branch length",
				"type": "range_float", "min": 0.08, "max": 0.34,
				"step": 0.005, "fallback": [0.17, 0.28]},
			{"key": "branch_forks", "label": "Forks per branch",
				"type": "range_int", "min": 1, "max": 3, "step": 1,
				"fallback": [1, 2]},
			{"key": "raised_offshoot_chance", "label": "Raised offshoot chance",
				"type": "float", "min": 0.0, "max": 1.0, "step": 0.01},
			{"key": "litter_spacing", "label": "Litter spacing",
				"type": "float", "min": 0.04, "max": 0.20,
				"step": 0.005},
		],
	},
	"clay_sprouts": {
