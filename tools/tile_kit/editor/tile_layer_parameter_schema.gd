...
const KIND_ORDER := [
	...
	"sprite_tufts", "clay_sprouts", "fringe", "fence",
]
...
const DEFINITIONS := {
		...
		"parameters": [
			...
			{"section": "SURFACE RELIEF", "key": "relief_style", "label": "Relief style",
				"type": "enum", "values": ["none", "pillow", "dunes",
					"sculpted_dunes", "contour_moss", "heaps", "furrows"],
				"value_labels": ["None", "Soft pillow", "Dunes", "Sculpted dunes",
					"Clay moss contours", "Heaps", "Furrows"], "randomize": false},
			{"key": "relief_amplitude", "label": "Relief amount", "type": "float",
				"min": 0.0, "max": 0.20, "step": 0.005,
				"when": {"relief_style": ["pillow", "dunes", "sculpted_dunes", "contour_moss", "heaps", "furrows"]}},
			...
			{"key": "dune_direction_degrees", "label": "Wind direction", "type": "float",
				...
				"when": {"relief_style": ["sculpted_dunes"]}},
			{"section": "CLAY MOSS CONTOURS", "key": "contour_bands",
				"label": "Shelf count", "type": "int", "min": 2, "max": 5,
				"step": 1, "when": {"relief_style": ["contour_moss"]}},
			{"key": "contour_edge_roll", "label": "Shelf edge roll", "type": "float",
				"min": 0.015, "max": 0.18, "step": 0.005,
				"when": {"relief_style": ["contour_moss"]}},
			{"key": "contour_warp", "label": "Flow variation", "type": "float",
				"min": 0.0, "max": 1.0, "step": 0.01,
				"when": {"relief_style": ["contour_moss"]}},
			{"key": "contour_axis", "label": "Flow direction", "type": "enum",
				"values": [0, 1], "value_labels": ["North / south", "East / west"],
				"randomize": false, "when": {"relief_style": ["contour_moss"]}},
	...
	"grass_clusters": {
		...
		"defaults": {"coverage_mode": "tufts", "tuft_scale": 1.0,
			...
			"accent_clumps": [1, 2], "moss_patch_style": "gg_cushion_moss",
		...
		"parameters": [
			{"section": "CARPET MODE", "key": "coverage_mode", "label": "Composition",
				"type": "enum",
				"values": ["gold_grass", "tufts", "moss_pads", "turf", "carpet", "clusters"],
				"randomize": false},
			{"section": "MOSS COMPOSITION", "key": "moss_patch_style",
				"label": "Material concept",
				"type": "enum",
				"values": ["gg_rosette_moss", "gg_cloud_bank_moss",
					"gg_mossy_garden_ground", "gg_chenille_moss", "gg_sheet_moss",
					"gg_cushion_moss", "gg_rolling_moss"],
				"value_labels": ["Pincushion rosettes", "Cloud banks",
					"Moss, soil, stone & grass", "Chenille carpet", "Sheet moss",
					"Cushion moss", "Rolling moss"],
				"randomize": false,
				"when": {"coverage_mode": ["moss_pads"]}},
			{"key": "moss_detail_scale", "label": "Growth scale",
				"type": "float", "min": 0.6, "max": 1.4, "step": 0.01,
				"when": {"coverage_mode": ["moss_pads"]}},
			{"key": "moss_detail_height", "label": "Growth height",
				"type": "float", "min": 0.5, "max": 1.6, "step": 0.01,
				"when": {"coverage_mode": ["moss_pads"]}},
			{"key": "moss_detail_density", "label": "Growth density",
				"type": "float", "min": 0.5, "max": 1.25, "step": 0.05,
				"when": {"coverage_mode": ["moss_pads"]}},
	...
	"clay_sprouts": {
		"label": "Clay Sprouts & Blooms",
		"description": "Plasticine leaf-pair sprouts, tiny shoots, flowers, and berries.",
		"defaults": {"leaf_key": "clay_leaf",
			"petal_key": "clay_flower_cream",
			"flower_centre_key": "clay_flower_gold",
			"berry_key": "clay_berry",
			"sprout_count": [7, 9], "shoot_count": [4, 6],
			"flower_count": [2, 4], "berry_count": [2, 3],
			"leaves_per_sprout": [3, 5], "leaf_height": [0.175, 0.215],
			"leaf_width": [0.125, 0.155], "leaf_depth_ratio": 0.72,
			"splay_degrees": [17.0, 26.0], "edge_margin": 0.16,
			"min_spacing": 0.42, "root_sink": 0.02,
			"mound_key": "grass_gg_top", "mound_height": 0.055,
			"mound_spread": 0.95},
		"parameters": [
			{"section": "COMPOSITION", "key": "sprout_count",
				"label": "Leaf sprouts", "type": "range_int", "min": 0,
				"max": 16, "step": 1, "fallback": [7, 9]},
			{"key": "shoot_count", "label": "Tiny shoots", "type": "range_int",
				"min": 0, "max": 12, "step": 1, "fallback": [4, 6]},
			{"key": "flower_count", "label": "Flowers", "type": "range_int",
				"min": 0, "max": 8, "step": 1, "fallback": [2, 4]},
			{"key": "berry_count", "label": "Berry clusters", "type": "range_int",
				"min": 0, "max": 8, "step": 1, "fallback": [2, 3]},
			{"key": "min_spacing", "label": "Spacing", "type": "float",
				"min": 0.15, "max": 0.70, "step": 0.01},
			{"key": "edge_margin", "label": "Rim margin", "type": "float",
				"min": 0.0, "max": 0.35, "step": 0.005},
			{"section": "LEAF SHAPE", "key": "leaves_per_sprout",
				"label": "Leaves per sprout", "type": "range_int", "min": 2,
				"max": 7, "step": 1, "fallback": [3, 5]},
			{"key": "leaf_height", "label": "Leaf height", "type": "range_float",
				"min": 0.06, "max": 0.34, "step": 0.005, "fallback": [0.15, 0.20]},
			{"key": "leaf_width", "label": "Leaf width", "type": "range_float",
				"min": 0.03, "max": 0.18, "step": 0.005, "fallback": [0.075, 0.10]},
			{"key": "leaf_depth_ratio", "label": "Leaf flatness", "type": "float",
				"min": 0.2, "max": 1.0, "step": 0.02},
			{"key": "splay_degrees", "label": "Leaf splay", "type": "range_float",
				"min": 5.0, "max": 55.0, "step": 1.0, "fallback": [15.0, 28.0]},
			{"key": "root_sink", "label": "Root sink", "type": "float",
				"min": 0.0, "max": 0.08, "step": 0.005},
			{"section": "ROOT TURF", "key": "mound_height",
				"label": "Turf mound height", "type": "float",
				"min": 0.0, "max": 0.12, "step": 0.005},
			{"key": "mound_spread", "label": "Turf mound spread",
				"type": "float", "min": 0.4, "max": 1.8, "step": 0.05},
		],
	},
	"fringe": {
