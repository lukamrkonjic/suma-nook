...
#
# Five stops, dark to bright (reference 9F9189 / C4B5A7 / E6D5BD /
...
var _cloud_top_y := -19.0
var _cloud_bottom_y := -30.5
...
func _process(delta: float) -> void:
	...
	_current_deep = _current_deep.lerp(_target_deep, blend)
	_current_mid = _current_mid.lerp(_target_mid, blend)
	...
	_material.set_shader_parameter("deep_shadow_color", _current_deep)
	_material.set_shader_parameter("mid_color", _current_mid)
...
func _build_cloud_sea() -> void:
	...
	# Screen-right for the default camera yaw: the sea drifts left to
	# right across the frame in a seamless endless cycle.
	_material.set_shader_parameter(
		"wind_direction",
...
func _refresh_palette_targets() -> void:
	...
	_target_shade = shade * grade
	_target_deep = deep * grade
	_target_mid = mid * grade
