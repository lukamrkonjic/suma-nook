...
func _present_tree_hit(instance_id: int, hit: Dictionary) -> void:
	...
	var point := renderer.structure_effect_position(instance_id, 0.65)
	if visual == null:
		renderer.animate_scalable_structure_harvest_impact(
			instance_id,
			float(hit.get("progress", 0.0)),
			bool(hit.get("final", false)),
			"clay_tree",
			func() -> void:
...
func _present_berry_gather(instance_id: int, hit: Dictionary) -> void:
	...
	var visual := renderer.structure_node(instance_id)
	var point := renderer.structure_effect_position(instance_id, 0.38)
	if visual == null:
		renderer.animate_scalable_structure_harvest_impact(
			instance_id,
			float(hit.get("progress", 0.0)),
...
func _present_rock_hit(instance_id: int, hit: Dictionary) -> void:
	...
	var visual := renderer.structure_node(instance_id)
	var point := renderer.structure_effect_position(instance_id, 0.24)
	if visual == null:
		renderer.animate_scalable_structure_harvest_impact(
			instance_id,
			float(hit.get("progress", 0.0)),
