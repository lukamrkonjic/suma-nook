...
func _ready() -> void:
	...
	game.grid.make_initial_island(2)
	...
	if failures.is_empty():
		print("TILEGARDEN FULL LOOP PASSED — %d scene-level assertions" % assertions)
		_quit_clean(0)
	else:
...
func _exercise_undo_save() -> void:
	...
	game.environment_style.set_preset(&"soft_rain")
	...
	check(str(loaded.environment_preset) == "soft_rain", "lighting/background selection survives save serialization")
